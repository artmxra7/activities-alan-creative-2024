      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex items-center mb-4">
            <label className="text-gray-800 mr-2">Filter by Element:</label>
            <select
              className="p-2 bg-gray-200 text-gray-800 rounded"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {displayedPokemonList.map((pokemon) => (
              <div key={pokemon.name} className="cursor-pointer relative" onClick={() => handleSelectPokemon(pokemon.name)}>
                <div className={`bg-gray-300 p-4 rounded-md flex flex-col items-center ${expandedPokemon === pokemon.name ? 'expanded' : ''}`} onClick={() => handleExpand(pokemon.name)}>
